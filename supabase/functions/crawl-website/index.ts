...
async function performCrawl(supabaseClient: any, website: any, scanId: string) {
  try {
    ...
    console.log('Starting URL discovery and analysis...')
    ...
    for (const url of urlsToCrawl) {
      try {
        ...
        const pageResult = await analyzePageAccessibility(url)
    ...
    console.log(`Crawl completed. Analyzed ${scannedCount} pages, found ${totalIssues} issues`)
...
async function discoverUrls(baseUrl: string, maxDepth: number, maxPages: number): Promise<string[]> {
  ...
  while (queue.length > 0 && discovered.size < maxPages) {
    ...
    try {
      ...
      if (depth < maxDepth) {
        // Extract links from HTML using regex (simple approach)
        ...
        while ((match = linkRegex.exec(html)) !== null) {
          try {
            ...
            if (linkDomain === baseDomain && 
                !discovered.has(linkUrl) && 
                !queue.some(item => item.url === linkUrl)) {
...
async function analyzePageAccessibility(url: string): Promise<PageResult> {
  ...
  try {
    ...
    const issues = performAccessibilityChecks(html, url)
...
function performAccessibilityChecks(html: string, url: string): AccessibilityIssue[] {
  ...
  const inputRegex = /<input[^>]*type=["'](?:text|email|password|tel|search)["'][^>]*>/gi
  let inputMatch
    ...
    if (idMatch) {
      ...
      if (!labelRegex.test(html)) {
        issues.push({
          ...
          helpText: 'Add a label element that references this input',
  ...
  if (!html.match(/<title[^>]*>[^<]+<\/title>/i)) {
    issues.push({
      ...
      helpText: 'Add a descriptive title to the page',
  ...
  if (!html.match(/<html[^>]*lang=["'][^"']+["']/i)) {
    issues.push({
      ...
      helpText: 'Add a lang attribute to the html element',
  ...
  if (!h1Matches || h1Matches.length === 0) {
    issues.push({
      ...
      helpText: 'Add an h1 element to provide the main heading for the page',
      helpUrl: 'https://www.w3.org/WAI/WCAG21/Understanding/headings-and-labels.html',
      targetElement: 'body',
      htmlSnippet: 'No h1 found'
    })
  }
  
  // Check for low contrast (simplified check for obvious cases)
  const colorRegex = /color:\s*(#(?:000|111|222|333)(?![0-9a-f])|black|#000000)/gi
  const bgColorRegex = /background(?:-color)?:\s*(#(?:fff|eee|ddd|ccc)(?![0-9a-f])|white|#ffffff)/gi
  if (colorRegex.test(html) && bgColorRegex.test(html)) {
    issues.push({
      ruleId: 'color-contrast',
      impact: 'serious',
      description: 'Elements must have sufficient color contrast',
      helpText: 'Ensure that text and background colors have sufficient contrast ratio',
      helpUrl: 'https://www.w3.org/WAI/WCAG21/Understanding/contrast-minimum.html',
      targetElement: 'text elements',
      htmlSnippet: 'Low contrast detected'
    })
  }
